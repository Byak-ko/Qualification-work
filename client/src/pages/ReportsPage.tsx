...
import { Rating, RatingStatus } from '../types/Rating';
import { DocumentTextIcon, ChartPieIcon, ChevronDownIcon, ArrowPathIcon, TableCellsIcon } from '@heroicons/react/24/outline';
...
const ReportsPage: React.FC = () => {
    ...
    const fetchRatings = async () => {
      if (currentUser) {
        try {
          ...
          setRatings(data.filter((rating: reportRating) => rating.author.id === currentUser.id));
        } catch (error) {
  ...
  // Обчислюємо загальні підсумки для всіх груп
  const summary = useMemo(() => {
    ...
    return {
      ...
      averageScore: report.reduce((sum, group) => sum + (group.averageScore * group.totalParticipants), 0) / 
                    report.reduce((sum, group) => sum + group.totalParticipants, 0) || 0,
      highestScoreGroup: report.reduce((highest, group) => 
                        group.averageScore > highest.score 
                          ? { name: group.name, score: group.averageScore } 
                          : highest, 
                        { name: '', score: 0 }),
      lowestScoreGroup: report.reduce((lowest, group) => 
        ...
        <div className="flex flex-wrap gap-4 p-6">
          <div className="w-full md:w-64">
            ...
            <div className="relative">
              <select
                className="appearance-none block w-full bg-white border border-gray-300 rounded-lg px-4 py-3 pr-8 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          ...
          <div className="w-full md:w-64">
            ...
            <div className="relative">
              <select
                className={`appearance-none block w-full bg-white border border-gray-300 rounded-lg px-4 py-3 pr-8 shadow-sm ${
                  !selectedRating 
            ...
            <div key={index} className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
              ...
              <div className="p-6">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4 shadow-sm">
                    <p className="text-sm text-gray-500 mb-1">Заповнено</p>
                  ...
                  <div className="bg-gradient-to-br from-red-50 to-rose-50 rounded-xl p-4 shadow-sm">
                    <p className="text-sm text-gray-500 mb-1">Очікують заповнення</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl p-5 shadow-md">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600">Очікують заповнення:</span>
                        <div className="flex items-center">
                ...
                <div className="mb-6">
                  ...
                  <div className="h-4 w-full bg-gray-200 rounded-full overflow-hidden">
                    <div className="flex h-full">
                      ...
                      <div 
                        ...
                        title={`Очікують заповнення: ${summary.totalPending}`}
                      ></div>
                    </div>
                  </div>
                  <div className="flex flex-wrap mt-2 text-xs gap-4">
                    ...
                    <div className="flex items-center">
                      ...
                      <span>Очікують заповнення ({((summary.totalPending / summary.totalParticipants) * 100).toFixed(1)}%)</span>
                    </div>
                  </div>
                </div>
                          ...
                          <td className="px-6 py-3 whitespace-nowrap">
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div 
                                ...
                                style={{ width: `${((group.filledCount + group.approvedCount) / group.totalParticipants) * 100}%` }}
                              ></div>
                            </div>
                            <span className="text-xs text-gray-500 mt-1 block">
                              {(((group.filledCount + group.approvedCount) / group.totalParticipants) * 100).toFixed(1)}%
                            </span>
                          </td>
                        </tr>
                      ))}
                      ...
                      <tr>
                        ...
                        <td className="px-6 py-3 whitespace-nowrap">
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div 
                              className="bg-blue-600 h-2 rounded-full"
                              style={{ width: `${((summary.totalFilled + summary.totalApproved) / summary.totalParticipants) * 100}%` }}
                            ></div>
                          </div>
                          <span className="text-xs text-gray-500 mt-1 block">
                            {(((summary.totalFilled + summary.totalApproved) / summary.totalParticipants) * 100).toFixed(1)}%
                          </span>
                        </td>
                      </tr>
                    </tfoot>
